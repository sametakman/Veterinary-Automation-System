    ...
    public partial class Form19 : Form
    {
                        ...
                        if (selectedRow.DataBoundItem is DataRowView rowView && rowView.Row != null)
                        {
                            ...
                            if (result == DialogResult.Yes)
                            {
                                ...
                                using (SQLiteConnection conn = new SQLiteConnection(connectionString))
                                {
                                    ...
                                    {
                                        string deleteQuery = "DELETE FROM kopek_gebelik WHERE ID = @ID";
        ...
        private async void Form19_Load(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    ShowSystemTrayNotification("Bilgi", "Veri bulunamadı. kopek_gebelik tablosunda kayıt yok.", ToolTipIcon.Info);
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Form19_Load Hata: {ex.Message}\nStackTrace: {ex.StackTrace}");
            }
            finally
            {
                isLoading = false;
                        ...
                        using (SQLiteDataAdapter da = new SQLiteDataAdapter(cmd))
                        {
                            ...
                            if (dt.Rows.Count == 0)
                            {
                                ShowSystemTrayNotification("Bilgi", string.IsNullOrWhiteSpace(filter) ?
                                    "Veri bulunamadı. kopek_gebelik tablosunda veri olmayabilir." :
            ...
            try
            {
                ...
                using (SmtpClient smtpClient = new SmtpClient("smtp.gmail.com", 587))
                {
                    ...
                    using (MailMessage mailMessage = new MailMessage())
                    {
                        ...
                        mailMessage.Subject = $"Doğum Uyarısı - ID: {row.Cells["ID"].Value?.ToString() ?? "Bilinmeyen"}";
                ...
                PlayNotificationSound(@"C:\Users\ridva\Desktop\dutdut\my-project\veteriner_otomasyon\kopek.wav");
            }
            catch (Exception ex)
            {
        ...
        private async void button1_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                using (SQLiteConnection con = new SQLiteConnection(connectionString))
                {
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM kopek_gebelik WHERE Telefon = @Telefon";
                        ...
                        string insertQuery = @"
                            INSERT INTO kopek_gebelik (Telefon, Cinsi, HayvanSahibi, GebelikDurumu, KayitTarihi, DogumTarihi, DogumaKalanSure)
            ...
            try
            {
                ...
                using (SQLiteConnection con = new SQLiteConnection(connectionString))
                {
                    ...
                    using (SQLiteTransaction transaction = con.BeginTransaction())
                    {
                        ...
                        string checkQuery = "SELECT COUNT(*) FROM kopek_gebelik WHERE Telefon = @Telefon AND ID != @ID";
                        using (SQLiteCommand checkCmd = new SQLiteCommand(checkQuery, con, transaction))
                        {
                            checkCmd.Parameters.AddWithValue("@Telefon", telefon);
        ...
        {
            this.Hide();
            Form21 form21 = Application.OpenForms["Form21"] as Form21;
            if (form21 != null)
            {
                form21.Show();
            }
